...
export const songCacheKey = (songId: number | string): string => `song:${songId}`;
export const albumSongsCacheKey = (albumId: number | string): string => `songs:album:${albumId}`;
export const albumCacheKey = 'albums';
export const songsCacheKey = 'songs';
...
export const getAllSongsOfAlbum = TryCatch(async (req: Request, res: Response) => {
  ...
  const result = await pool.query('SELECT * FROM songs WHERE album_id = $1 ORDER BY created_at DESC', [albumId]);
